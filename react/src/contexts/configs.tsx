...
export const ConfigsProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const shouldShowLogin =
      (llmModels.length === 0 || toolList.length === 0) && modelList?.errorType === 'auth'
      ...
      if (modelList.errorType === 'network') {
        toast.error('Network connection failed', {
          ...
          action: {
            label: 'Retry',
            onClick: () => {
              console.log('User manually retrying model loading')
              refreshModels()
            },
          },
          duration: 8000,
        ...
        modelList.errorType === 'unknown' &&
        (llmModels.length === 0 || toolList.length === 0)
      ) {
