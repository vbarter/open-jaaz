...
class GenerateVideoBySora2InputSchema(BaseModel):
    prompt: str = Field(
...
@tool("generate_video_by_sora2_yunwu",
      description="Generate high-quality videos using Sora2 model. Fast text-to-video generation with optimized performance.",
      args_schema=GenerateVideoBySora2InputSchema)
async def generate_video_by_sora2_yunwu(
    prompt: str,
    config: RunnableConfig,
    tool_call_id: Annotated[str, InjectedToolCallId],
) -> str:
    """
    Generate a video using Sora2 model via yunwu service
    """
    print(f'🛠️ Sora2 Video Generation tool_call_id: {tool_call_id}')
    ...
    await send_to_websocket(session_id, {
        ...
        'id': tool_call_id,
        'name': 'generate_video_by_sora2_yunwu',
        'arguments': json.dumps(arguments)
    })

    # 等待确认
    confirmed = await tool_confirmation_manager.request_confirmation(
        tool_call_id, session_id, 'generate_video_by_sora2_yunwu', arguments
    )
